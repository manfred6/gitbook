...
To achieve this in a reproducable, auditable and scalable way, we leverage [Hashi Corp Terraform](). 
...
The source code for this particular setup can be found in the dedicated [ares-detection]() repository, under the `./terraform/` directory.
...
The terraform configuration is set up such that I can simply add nodes in the [variables.tf]() file, and they get deployed in a predictable way into my proxmox environment. This can be done as such:
...
Other settings such as cpu assignments, disk size, memory size, etc are defined cluster wide (also in variables.tf). This can me migrated to a per-node configuration later on, but i dont see it as necessary for now.


## Creating the Cluster

I have provided a helper-script in [scripts/init.sh]() to help instantiate the cluster:
...
Terraform also writes the ansible `hosts.ini`, which is configured in [outputs.tf]():
...
This same file also generates a corresponding `ansible.cfg`, including settings for where ansible can find the private key generated by terraform:
